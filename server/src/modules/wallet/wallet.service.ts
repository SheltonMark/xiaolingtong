...
export class WalletService {
  ...
  async withdraw(userId: number, amount: number) {
    ...
    try {
      ...
      return this.buildPendingWithdrawResponse(
        user,
        tx,
        false,
        bill,
  ...
  private getWithdrawDisplayMessage(reason?: string) {
    ...
    if (
      ...
      && (message.includes('IP地址不允许调用接口') || message.includes('IP'))
    ) {
      return '微信提现未配置服务器IP白名单，请联系管理员处理';
    }
  ...
  private async findLatestPendingWithdraw(userId: number) {
    ...
    return (
      pendingTxs.find(
        (item: WalletTransaction) => item?.status === 'pending' && item?.refType,
      ) || null
    );
  }
